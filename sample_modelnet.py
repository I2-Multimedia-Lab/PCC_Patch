...
def process(path, args):
    try:
        ...
        pc = pc_mesh.get_sample("mesh_random", n=args.n_point, as_PyntCloud=True)
        coords = ['x', 'y', 'z']
        points = pc.points.values
        points = points - np.min(points)
        points = points / np.max(points)
        #points = points * (args.vg_size - 0.0001)
        if args.quantize:
...
if __name__ == '__main__':
    ...
    parser.add_argument('source', help='Source directory', default='./data/ModelNet40')
    parser.add_argument('dest', help='Destination directory', default='./data/ModelNet40_pc_8192')
    parser.add_argument('--vg_size', type=int, help='Voxel Grid resolution for x, y, z dimensions', default=1)
    parser.add_argument('--n_point', type=int, help='Number of samples', default=8192)
    parser.add_argument('--quantize', type=bool, help='Whether to round the numbers', default=False)
    ...
    assert args.n_point > 0, f'n_point must be positive'
