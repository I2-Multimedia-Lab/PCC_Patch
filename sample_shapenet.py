...
def main(args):
    ...
    if not os.path.exists(args.dest):
        os.makedirs(os.path.join(args.dest, 'test'))

    #TRAIN_DATASET = PartNormalDataset(root=args.source, npoints=args.n_point, split='trainval', normal_channel=False)
    #trainDataLoader = torch.utils.data.DataLoader(TRAIN_DATASET, batch_size=1, shuffle=False, num_workers=1)
    TEST_DATASET = PartNormalDataset(root=args.source, npoints=args.n_point, split='test', normal_channel=False)
    ...
    for i, data in tqdm(enumerate(testDataLoader), total=len(testDataLoader), smoothing=0.9):
        ...
        without normal:
            points (B, N, 3)
            label (B, 1), class of the whole point cloud
            target (B, N), the part class(0~49) of each point
        '''
        points = points.data.numpy()

        #points = points.transpose(2, 1) # points: (B, 3, N)
        B, N, C = points.shape
        pc = points.reshape((N, C))
        pc = pc + 0.5   

        pc_io.save_pc(pc, f'{i}.ply', os.path.join(args.dest, 'test'))
